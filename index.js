...
function addKeyValuePairs(dictionary, entries) {
  ...
  entries.forEach(([key, value]) => {
    if (!dictionary.has(key)) {
      dictionary.set(key, value);
      added++;
      //   dictionary.set("added", added);
    } else {
      rejected++;
      //   dictionary.set("rejected", rejected);
    }
  });
...
function checkPresence(dictionary, keys) {
  ...
  keys.forEach((elem, index, array) => {
    if (dictionary.has(elem)) {
      presence.push(dictionary.has(elem));
    } else {
      presence.push(dictionary.has(elem));
    }
  });
...
function sortByValues(dictionary) {
  ...
  let convArr = Array(...dictionary.entries());

  convArr.sort((a, b) => b[1] - a[1]);

  return new Map(convArr);
...
function convertSetToDictionary(set) {
  ...
  set.forEach((key, value) => newDictionary.set(key, value.codePointAt(0)));
